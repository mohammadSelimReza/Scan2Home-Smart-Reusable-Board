...
class AgentReview(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='agent_reviews_given')
    ...
    def __str__(self):
        agent_name = self.agent.brand_name or self.agent.user.full_name
        return f'{self.rating} stars by {self.user.full_name} for {agent_name}'
...
def update_agent_rating(sender, instance, **kwargs):
    agent_profile = instance.agent
    reviews = agent_profile.reviews.all()
    if reviews.exists():
        avg_rating = reviews.aggregate(Avg('rating'))['rating__avg']
        agent_profile.rating = round(avg_rating, 1)
        agent_profile.rating_count = reviews.count()
    else:
        agent_profile.rating = 0.0
        agent_profile.rating_count = 0
    agent_profile.save(update_fields=['rating', 'rating_count'])
